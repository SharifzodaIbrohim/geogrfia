...
def install(app=None):
    ...
    def olympiad_start(olympiad_id):
        try:
            ...
            # Attach olympiad title for student UI
            session["title"] = oly.get("title") or oly.get("name") or ""
            session["olympiadTitle"] = session["title"]

            # Persist real student name/class/school on attempt (results/monitor)
            try:
                from db.repo import find_student_by_code
                from db.connection import get_session, is_postgres_enabled
                from sqlalchemy import text
                st = find_student_by_code(student_id)
                if st:
                    nm = (st.get("fullName") or st.get("full_name") or "").strip()
                    cl = (st.get("className") or st.get("class_name") or "").strip()
                    sch = (st.get("school") or st.get("school_name") or "").strip()
                    code = st.get("id") or st.get("student_code") or student_id
                    if nm:
                        session["studentName"] = nm
                        session["fullName"] = nm
                        session["studentCode"] = code
                    aid = session.get("attemptId") or session.get("sessionId") or session.get("id")
                    if aid and is_postgres_enabled():
                        with get_session() as s:
                            s.execute(
                                text(
                                    "UPDATE attempts SET "
                                    "student_name = COALESCE(NULLIF(:n, ''), student_name), "
                                    "student_class = COALESCE(NULLIF(:c, ''), student_class), "
                                    "student_school = COALESCE(NULLIF(:sch, ''), student_school) "
                                    "WHERE id::text = :id"
                                ),
                                {"n": nm or student_id, "c": cl or None, "sch": sch or None, "id": str(aid)},
                            )
                            try:
                                s.commit()
                            except Exception:
                    ...
                    aid = session.get("attemptId") or session.get("sessionId")
                    if aid and is_postgres_enabled():
                        with get_session() as s:
                            s.execute(
                                text(
                                    "UPDATE attempts SET student_name = COALESCE(NULLIF(student_name, ''), :n) "
                                    "WHERE id::text = :id"
                                ),
                                {"n": str(student_id), "id": str(aid)},
                            )
                            try:
                                s.commit()
                            except Exception:
                ...
                log.warning("start name enrich failed: %s", e)

            safe = {}
            for k, v in session.items():
                if k in ("scoreMap", "answerKey", "correctMap"):
                    continue
                try:
                    jsonify({k: v})
                    safe[k] = v
    ...
    def _do_submit(olympiad_id):
        try:
            ...
            # Enrich result name if missing
            try:
                if student_id and isinstance(result, dict) and not result.get("studentName"):
                    from db.repo import find_student_by_code
                    st = find_student_by_code(student_id)
                    if st:
                        result["studentName"] = st.get("fullName") or st.get("full_name")
                        result["fullName"] = result["studentName"]
            except Exception:
                pass
            return jsonify(result if isinstance(result, dict) else {"ok": True, "result": result})
        except ValueError as e:
            code = str(e)
            return jsonify({"error": code, "reason": code}), 400
        except Exception as e:
