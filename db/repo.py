...
def list_students() -> list[dict]:
    ...
    return [st for st in _load_json(STUDENTS_FILE) if (st.get("status") or "active") == "active"]
...
def find_student_by_code(code: str) -> dict | None:
    ...
    for st in _load_json(STUDENTS_FILE):
        if st.get("id") == code and (st.get("status") or "active") == "active":
            return st
    return None
...
def soft_delete_student(student_id: str) -> bool:
    """Mark student inactive (soft delete). Keeps FK history intact."""
    ...
    if use_pg():
        with get_session() as s:
            r = s.execute(
                text("UPDATE students SET status = 'inactive' WHERE student_code = :c AND status = 'active'"),
                {"c": code},
            )
    ...
    found = False
    for st in items:
        if str(st.get("id") or st.get("student_code") or "") == code:
            st["status"] = "inactive"
            found = True
            break
    if found:
...
def delete_student(student_id: str) -> bool:
    """Soft-delete preferred (alias)."""
    return soft_delete_student(student_id)


def update_student(student_id: str, fields: dict) -> bool:
    code = (student_id or "").strip()
    ...
    if use_pg():
        ...
        mapping = {
            ...
            "school": "school_name", "school_name": "school_name",
            "status": "status",
        }
        ...
        if not sets:
            return False
        with get_session() as s:
            r = s.execute(text(f"UPDATE students SET {', '.join(sets)} WHERE student_code = :c"), params)
            return (r.rowcount or 0) > 0
    items = _load_json(STUDENTS_FILE)
    found = False
    for st in items:
        if str(st.get("id") or "") == code:
            st.update(fields)
            found = True
            break
    if found:
