(() => {
  ...
  function loginErrText(err) {
    const msg = (err && err.message) ? String(err.message) : '';
    const status = err && err.status;
    if (status === 0 || /failed to fetch|networkerror|load failed|шабака|network/i.test(msg)) {
      return 'Хатои шабака. Пайвастро санҷед ва саҳифаро навсозӣ кунед.';
    }
    if (status === 401 || status === 403 || /парол|password|login|нодуруст|unauthorized|рад|invalid|credentials/i.test(msg)) {
      return 'Логин ё парол нодуруст аст.';
    }
    return msg || 'Хато рух дод';
  }
  async function api(path, options = {}) {
    const headers = { 'Content-Type': 'application/json', ...(options.headers || {}) };
    if (token) { headers['X-Admin-Token'] = token; headers['Authorization'] = 'Bearer ' + token; }
    let res;
    try {
      res = await fetch(API + path, { ...options, headers, credentials: 'include' });
    } catch (net) {
      const e = new Error('Хатои шабака. Пайвастро санҷед ва саҳифаро навсозӣ кунед.');
      e.status = 0;
      throw e;
    }
    const data = await res.json().catch(() => ({}));
    if (!res.ok) {
      const err = new Error(data.error || data.message || 'Хато');
  ...
  loginForm?.addEventListener('submit', async (e) => {
    ...
    const loginEl = document.getElementById('adminLogin') || document.querySelector('[name="login"]');
    const passEl = document.getElementById('adminPassword') || document.querySelector('[name="password"]');
    const loginVal = (loginEl && loginEl.value || '').trim();
    const passVal = (passEl && passEl.value) || '';
    if (!loginVal || !passVal) {
      if (loginError) {
        loginError.textContent = 'Логин ва паролро нависед.';
    ...
    try {
      ...
      if (loginError) { loginError.textContent = loginErrText(err); loginError.classList.remove('hidden'); }
    }
  });
  ...
  function displayCode(r) {
    var c = r.studentCode || r.studentId || r.student_id || r.code || r.id || '';
    c = String(c || '').trim();
    // Prefer long numeric student codes over attempt UUIDs
    if (/^[0-9]{6,}$/.test(c)) return c;
    var alt = r.student_code || r.code || '';
    if (/^[0-9]{6,}$/.test(String(alt))) return String(alt);
    return c || '—';
  }
  function displayName(r) {
    var n = (r.studentName || r.name || r.fullName || r.student_name || '').trim();
    if (n && !/^\d{8,}$/.test(n) && n !== '—' && n.toLowerCase() !== 'gmail user') return n;
    if (r.lastName || r.firstName) {
      var joined = [r.lastName, r.firstName, r.middleName || r.patronymic].filter(Boolean).join(' ').trim();
      ...
      if (liveBody) {
        ...
        liveBody.innerHTML = rows.length
          ? rows.map((r) => `<tr>
              ...
              <td>${r.answered ?? '—'}</td>
              <td>${esc(fmtWhen(r.startedAt))}</td>
              <td>${esc(fmtWhen(r.expiresAt || r.endsAt))}</td>
            </tr>`).join('')
        ...
        recentBody.innerHTML = rows.length
          ? rows.map((r) => {
              ...
              return `<tr>
                ...
                <td>${esc(fmtWhen(r.finishedAt || r.time || r.submittedAt || r.createdAt))}</td>
              </tr>`;
            }).join('')
    ...
    try {
      ...
      body.innerHTML = rows.length
        ? rows.map((r) => {
          const st = statusLabel(r.status);
          const stClass = (String(r.status||'').toLowerCase()==='passed') ? 'ok' : ((String(r.status||'').toLowerCase()==='failed') ? 'error' : '');
          return `<tr data-attempt-id="${esc(r.id || r.attemptId || '')}">
            <td>${esc(displayName(r))}</td>
            <td><code>${esc(displayCode(r))}</code></td>
            <td>${esc(r.school || r.studentSchool || '—')}</td>
            <td>${esc(r.className || r.studentClass || '—')}</td>
            <td>${esc(formatScoreCell(r))}</td>
            <td class="${stClass}">${esc(st)}</td>
            <td>${esc(fmtWhen(r.finishedAt || r.submittedAt || r.createdAt))}</td>
          </tr>`;
        }).join('')
  ...
  window.__geoPortalReady = true;
  try {
    const hint = document.getElementById('jsLoadHint');
