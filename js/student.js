...
(function () {
  ...
  function loginErrText(e) {
    ...
    var reason = e && e.data && (e.data.reason || e.data.code);
    if (status === 0 || /шабака|failed to fetch|networkerror|load failed|network/i.test(msg)) {
      return 'Хатои шабака. Пайвастро санҷед ва саҳифаро навсозӣ кунед.';
    }
    if (status === 401 || status === 403 || status === 404 ||
        reason === 'student_not_found' ||
        /нодуруст|not found|invalid|unauthorized|рад шуд/i.test(msg)) {
      return 'ID нодуруст аст ё хонанда ёфт нашуд.';
    }
  ...
  function renderQuestion() {
    ...
    var qWord = t('question') || t('questionLabel');
    if (!qWord || qWord === 'question' || qWord === 'questionLabel' || /question/i.test(qWord) && qWord.length < 20 && qWord.indexOf('Савол') < 0 && qWord.indexOf('Вопрос') < 0 && qWord.indexOf('Question') < 0) qWord = 'Савол';
  ...
  function bindUI() {
    ...
    if (loginForm) {
      loginForm.addEventListener('submit', async function (ev) {
        ev.preventDefault();
        var inp = $('studentIdInput') || $('studentLogin') || $('studentId') || document.querySelector('[name="studentId"], [name="id"]');
        var id = (inp && inp.value || '').trim();
        var err = $('loginError');
        if (!id) {
          if (err) { err.textContent = 'ID-ро нависед.'; show(err, true); }
          return;
        }
        ...
        try {
          ...
          var text = loginErrText(e);
          if (err) { err.textContent = text; show(err, true); }
          else alert(text);
        }
      });
    }
  ...
  function boot() {
    try {
      init();
      window.__geoPortalReady = true;
      var hint = document.getElementById('jsLoadHint');
      if (hint) hint.classList.add('hidden');
    } catch (e) {
      console.error('[student]', e);
      var err = document.getElementById('loginError');
      if (err) {
        err.textContent = 'Саҳифаро навсозӣ кунед';
